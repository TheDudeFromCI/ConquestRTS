...
public class Chunk{
	...
	private static int blockIndex(int x, int y, int z){
		return x+z*BLOCKS_PER_CHUNK+y*BLOCKS_PER_CHUNK*BLOCKS_PER_CHUNK;
	}
	...
	public int getBlock(int x, int y, int z){
		return blocks[blockIndex(x&HIGH_BLOCK_COUNT, y&HIGH_BLOCK_COUNT, z&HIGH_BLOCK_COUNT)];
	}
	private boolean getSmallBlock(int x, int y, int z, BlockSideProperties props){
		...
		if(props==null||(block.textures.getTexture(props.side)==props.texture&&block.textures.getRotation(props.side)==props.rotation&&block.textures.colors[props.side*3]==props.r&&block.textures.colors[props.side*3+1]==props.g&&block.textures.colors[props.side*3+2]==props.b)) return block.shape.getBlock(x&7, y&7, z&7, block.rotation);
		return false;
	}
	...
	private void optimizeSideX(int x, int side, QuadBatch batch, BlockSideProperties props){
		int y, z, q;
		for(y = 0; y<SMALL_BLOCK_COUNT; y++)
			for(z = 0; z<SMALL_BLOCK_COUNT; z++)
