...
public class QuadOptimizer{
	...
	public static int optimize(int[][] storage, int[][] temp, boolean[][] quads, int width, int height){
		int x, y;
		for(x=0; x<width; x++)for(y=0; y<height; y++)temp[x][y]=storage[x][y];
		int xS = optimizeXStrong(storage, quads, width, height);
		int yS = optimizeYStrong(temp, quads, width, height);
		used.clear();
		if(xS<=yS)return xS;
		for(x=0; x<width; x++)for(y=0; y<height; y++)storage[x][y]=temp[x][y];
		return yS;
	}
	private static int optimizeXStrong(int[][] storage, boolean[][] quads, int width, int height){
		...
		int w = -1;
		int total = 0;
		for(x=0; x<width; x++)for(y=0; y<height; y++)storage[x][y]=-1;
		for(y=0; y<height; y++){
			w=-1;
			for(x=0; x<width; x++){
				if(!quads[x][y]){
				...
				if(!quads[x][y]){
					if(w>0){
						...
						if(!quads[x][y+1]&&(x==w||!quads[x-w-1][y+1])){
							...
							if(q==w)for(t=x-w; t<x; t++)storage[t][y+1]=storage[t][y];
						}
						w=0;
					}
					continue;
		...
		for(x=0; x<width; x++)for(y=0; y<height; y++)if(storage[x][y]>-1&&!used.contains(storage[x][y]))used.add(storage[x][y]);
		for(x=0; x<width; x++)for(y=0; y<height; y++)if(storage[x][y]>-1)storage[x][y]=used.indexOf(storage[x][y]);
		return used.size();
	}
		...
		for(x=0; x<width-1; x++){
				...
				if(!quads[x][y]){
					if(w>0){
						...
						if(!quads[x+1][y]&&(y==w||!quads[x+1][y-w-1])){
							...
							if(q==w)for(t=y-w; t<y; t++)storage[x+1][t]=storage[x][t];
						}
						w=0;
					}
					continue;
			...
			if(w>0){
				...
				if(y==w||!quads[x+1][y-w-1]){
					...
					if(q==w){
						for(t=y-w; t<y; t++)storage[x+1][t]=storage[x][t];
					}
				}
			}
		}
	...
	public static void countQuads(QuadCounter counter, int[][] storage, int width, int height, int quadCount){
		...
		else for(o=0; o<quadReferences.length; o++)quadReferences[o]=false;
		for(x=0; x<width; x++){
			for(y=0; y<height; y++){
				...
				for(w=x; w<width; w++)if(storage[w][y]!=o)break;
				for(h=y; h<height; h++)if(storage[x][h]!=o)break;
				w-=x;
				h-=y;
