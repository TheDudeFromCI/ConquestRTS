...
public class ServerList implements GameRenderer{
		...
		refreashButton = new GuiButton(gui, game.getFolder().getImage("Refreash Button.png"), true, new Runnable(){
			...
			public void run(){
				...
				for(int i = 0; i<list.getListSize(); i++)
					((ServerListSlot)list.getScrollPaneEntry(i)).refreash();
			}
		});
	...
	public GuiScrollPanel getScrollPanel(){
		return list;
	}
	public ServerListSlot getSelectedServer(){
		return selectedServer;
	}
	public void render(Graphics2D g, int x, int y, int width, int height){
		g.drawImage(gui.getPane(), x, y, null);
	}
	public void setSelectedServer(ServerListSlot selectedServer){
