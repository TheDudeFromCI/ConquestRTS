...
public class BlockIcon implements ChunklessBlockHolder{
	...
	public static final float BLOCK_ZOOM = 30;
	public static final float SPIN_SPEED = 10;
	public static float getX(int id){
		return ((Loop.screenRes.width-GuiHandler.HOTBAR_SLOT)/Loop.screenRes.width-0.5f+(GuiHandler.HOTBAR_SLOT/2f/Loop.screenRes.width))*(BlockIcon.BLOCK_ZOOM*Loop.screenRes.width/Loop.screenRes.height)*(id<10?1:-1);
	}
	public static float getY(int id){
		return (float)((((Loop.screenRes.height-GuiHandler.HOTBAR_SLOT*10.0)/2+GuiHandler.HOTBAR_SLOT*(9-id%10))/Loop.screenRes.height-0.5f+(GuiHandler.HOTBAR_SLOT/2f/Loop.screenRes.height))*BlockIcon.BLOCK_ZOOM);
	}
	...
	public void render(float x, float y, float z, float rx, float ry){
		...
		GL11.glRotatef(BlockIcon.BLOCK_PITCH+rx, 1, 0, 0);
		GL11.glRotatef(BlockIcon.BLOCK_YAW+yaw+ry, 0, 1, 0);
	...
	public void update(double time){
		yaw = (float)((time*BlockIcon.SPIN_SPEED)%360);
	}
}