...
public class Inventory{
	...
	private static float getX(double mouseX){
		return ((float)mouseX/Loop.screenRes.width-0.5f)*BLOCK_ZOOM*Loop.screenRes.width/Loop.screenRes.height;
	}
	private static float getX(int index){
		return -(COLS_SHOWN-1)*BLOCK_SPACING/2f+(index%REAL_COLS_SHOWN)*BLOCK_SPACING;
	}
	private static float getY(double mouseY){
		return ((float)mouseY/Loop.screenRes.height-0.5f)*BLOCK_ZOOM;
	}
	private static float getY(int index){
		return -(-(ROWS_SHOWN-1)*BLOCK_SPACING/2f+(index/REAL_COLS_SHOWN+1.5f)*BLOCK_SPACING);
	}
	...
	public void render(){
		...
		int lastShown = Math.min(scrollPosition+TOTAL_SHOWN, blocks.size());
		for(int i = scrollPosition; i<lastShown; i++)
			blocks.get(i).render(getX(i), getY(i));
