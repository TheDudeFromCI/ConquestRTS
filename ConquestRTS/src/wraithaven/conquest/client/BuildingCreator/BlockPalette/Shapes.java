...
import wraithaven.conquest.client.GameWorld.Voxel.BlockShapes.*;
...
public class Shapes{
	...
	public void render(){
		...
		float x = x();
		for(int i = scrollbarPosition; i<lastShown; i++)icons.get(i).render(x, y(i-scrollbarPosition), -5, 0, 0);
	}
	public void update(double time){
		int lastShown = Math.min(scrollbarPosition+BLOCKS_SHOWN, icons.size());
		for(int i = 0; i<lastShown; i++)icons.get(i).update(time);
	}
	...
	private static double distanceSquared(float x1, float y1, float x2, float y2){ return Math.pow(x1-x2, 2)+Math.pow(y1-y2, 2); }
}