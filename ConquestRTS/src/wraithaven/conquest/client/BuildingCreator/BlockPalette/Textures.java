...
public class Textures{
	...
	public void render(){
		...
		for(int i = scrollbarPosition*2; i<lastShown; i++)UI.renderElement(textures[i]);
		renderMain();
	}
	...
	public Texture getTexture(){ return textures[selectedTexture].texture; }
	public Texture getTexture(int id){ return textures[id].texture; }
	private float x(int id){ return (id-scrollbarPosition*2)/2*TEXTURE_TOTAL_SIZE/1024*width+51f/1024*width+(Loop.screenRes.width-width)/2; }
	private float y(int id){ return (1-id%2)*TEXTURE_TOTAL_SIZE/768*height+75f/768*height+(Loop.screenRes.height-height)/2; }
}