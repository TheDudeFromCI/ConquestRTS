...
public class ShaderProgram{
	...
	private ShaderProgram(String vertexShader, String geometryShader, String fragmentShader){
		...
		if(vsStatus!=GL11.GL_TRUE)throw new RuntimeException(GL20.glGetShaderInfoLog(vs));
		//Geometry Shader
		if(geometryShader!=null){
			...
			if(gsStatus!=GL11.GL_TRUE)throw new RuntimeException(GL20.glGetShaderInfoLog(gs));
		}else gs = -1;
		//Fragment Shader
		...
		if(fsStatus!=GL11.GL_TRUE)throw new RuntimeException(GL20.glGetShaderInfoLog(fs));
		//Compile Program
		program = GL20.glCreateProgram();
		GL20.glAttachShader(program, vs);
		GL20.glAttachShader(program, fs);
		if(geometryShader!=null)GL20.glAttachShader(program, gs);
		GL20.glLinkProgram(program);
		GL20.glValidateProgram(program);
		int pStatus = GL20.glGetProgrami(program, GL20.GL_LINK_STATUS);
		if(pStatus!=GL11.GL_TRUE)throw new RuntimeException(GL20.glGetProgramInfoLog(program));
	}
	public void bind(){
		if(GL11.glGetInteger(GL20.GL_CURRENT_PROGRAM)!=program)GL20.glUseProgram(program);
	}
