...
public class SecondaryLoop implements Runnable{
	...
	private void genChunk(File file, int x, int y, int z, ChunkHeightData heightData){
				...
				for(b = 0; b<66; b++){
					heights[a][b] =
						(a==0||b==0||a==65||b==65?machine.getGroundLevel(tempA, b-1+z):heightData.getHeight(
							tempA, b-1+z))-1;
					type = Math.random()>=0.5?Block.Grass.id():Block.Dirt.id();
					for(c = 0; c<66; c++)
						if(c-1+y<heights[a][b])
							blockData.setBlock(a-1, c-1, b-1, type);
					blockData.setBlock(a-1, heights[a][b]+1, b-1, type);
				}
			}
			MeshRenderer render = blockData.mesh(true);
		...
		}
		{
			...
			byteCount += grassLocations.size()*9*4+4;
			byteCount += 64*64*3;
	...
	// private void genChunk(File file, int x, int y, int z, ChunkHeightData
	// heightData){
	...
	// for(b = 0; b<66; b++){
	// heights[a][b] =
	// (a==0||b==0||a==65||b==65?machine.getGroundLevel(tempA,
	// b-1+z):heightData.getHeight(tempA, b
	// -1+z))-1;
	...
	// quad = quadList.get(i);
	// shade =
	// (byte)(quad.side==2?255:quad.side==3?130:quad.side==0||quad.side==1?200:180);
	// v0 =
	// vertices.indexOf(quad.data.get(0), quad.data.get(1), quad.data.get(2),
	// shade, quad.data.get(12),
	// quad.data.get(13), (byte)quad.data.get(20));
	// v1 =
	// vertices.indexOf(quad.data.get(3), quad.data.get(4), quad.data.get(5),
	// shade, quad.data.get(14),
	// quad.data.get(15), (byte)quad.data.get(20));
	// v2 =
	// vertices.indexOf(quad.data.get(6), quad.data.get(7), quad.data.get(8),
	// shade, quad.data.get(16),
	// quad.data.get(17), (byte)quad.data.get(20));
	// v3 =
	// vertices.indexOf(quad.data.get(9), quad.data.get(10), quad.data.get(11),
	// shade,
	...
	// // Load plantlife.
	// // ---
	// HashMap<EntityType,ArrayList<GrassTransform>> grassLocations = new
	// HashMap();
	// HashMap<EntityType,ArrayList<EntityTransform>> entityLocations = new
	// HashMap();
	// EntityType entity;
	// float humidity;
	// float tempature;
	...
	// GrassTransform loc =
	// new GrassTransform(tempA+0.5f, heightData.getHeight(tempA, tempB),
	// tempB+0.5f,
	...
	// type =
	// dictionary.randomEntity(heightData.getBiome(fx, fz),
	// machine.getGiantEntitySeed()+1, fx, fz);
	// if(type!=null)
	// giants.add(new EntityTransform(type, f[0],
	// type.groundHits.getGround(machine, heightData,
	...
	// // ---
	// BinaryFile bin = new
	// BinaryFile(vertices.size()*(5*4+2)+indices.size()*4+8+bytes+64*64*3);
