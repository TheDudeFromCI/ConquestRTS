...
class VertexStorage{
	...
	public int indexOf(float x, float y, float z, byte shade, float tx, float ty, byte texture, boolean grass){
		for(int i = 0; i<size; i++)
			if(vertices[i].getX()==x&&vertices[i].getY()==y&&vertices[i].getZ()==z
				...
				&&vertices[i].getTexture()==texture&&vertices[i].isGrass()==grass)
				return i;
		if(size==vertices.length)
			vertices = Arrays.copyOf(vertices, vertices.length+100);
		Vertex v = new Vertex(x, y, z, shade, tx, ty, texture, grass);
		vertices[size] = v;
		size++;
		return size-1;
	}
