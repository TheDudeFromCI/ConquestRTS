...
public class ChunkIO{
	...
	public void saveBlocks(){
		String worldFolder = WraithavensConquest.saveFolder+File.separatorChar+WraithavensConquest.currentGameUUID;
	...
	/**
	 * Loads all blocks at this specified chunk location.
	 * Returns true if blocks could be read. False otherwise.
	 * 
	 * If false, current block array will not change from it's previous state.
	 */
	...
	/**
	 * Gets the current block array from last load, or last block set. This array is instanced, not copied.
	 */
	public byte[] getBlocks(){
		return blocks;
	}
	/**
	 * Sets the current border blocks for this chunk. This is used for calculating occulision culling, and
	 * simplifiying the mesh for faster rendering.
	 */
	...
	public void saveMesh(){
		...
		BlockProperties d = new BlockProperties(Math.min(Chunk.BLOCKS_PER_CHUNK*Chunk.BLOCKS_PER_CHUNK, Block.values().length));
		for(int i = 0; i<=Chunk.CHUNK_BITS; i++)
		...
		for(a = 0; a<blockSize; a++)
			for(b = 0; b<blockSize; b++)
				for(c = 0; c<blockSize; c++){
					...
					if(!p.contains(type))p.place(type);
				}
	...
	private void saveLOD(int level, int[][] a, int[][] b, boolean[][] c, BlockProperties d,
			...
			if(j==0||j==1){
				for(x = 0; x<chunkSize; x++){
					...
					for(y = 0; y<chunkSize; y++)
						for(z = 0; z<chunkSize; z++){
							...
							if(!d.contains(block))d.place(block);
						}
					for(i = 0; i<d.size(); i++){
						for(y = 0; y<chunkSize; y++)
							for(z = 0; z<chunkSize; z++){
								shown = newBlocks[indexOf(x, y, z, chunkSize)]==d.get(i)
										&&isSideShown(x, y, z, j, newBlocks, blockSize, chunkSize);
								for(u = 0; u<blockSize; u++)
									for(v = 0; v<blockSize; v++)
										c[y*blockSize+u][z*blockSize+v] = shown;
							}
						q = QuadOptimizer.optimize(a, b, c, Chunk.BLOCKS_PER_CHUNK, Chunk.BLOCKS_PER_CHUNK, true);
						if(q==0)continue;
						xCounter.setup(this.x<<Chunk.CHUNK_BITS, this.y<<Chunk.CHUNK_BITS, this.z<<Chunk.CHUNK_BITS, x, j, listener, Block.values()[d.get(i)+Block.ID_SHIFT]);
				...
				for(y = 0; y<chunkSize; x++){
					...
					for(i = 0; i<d.size(); i++){
						...
						q = QuadOptimizer.optimize(a, b, c, Chunk.BLOCKS_PER_CHUNK, Chunk.BLOCKS_PER_CHUNK, true);
						if(q==0)continue;
						yCounter.setup(this.x<<Chunk.CHUNK_BITS, this.y<<Chunk.CHUNK_BITS, this.z<<Chunk.CHUNK_BITS, y, j, listener, Block.values()[d.get(i)+Block.ID_SHIFT]);
				...
				for(z = 0; z<chunkSize; z++){
					...
					for(i = 0; i<d.size(); i++){
						...
						q = QuadOptimizer.optimize(a, b, c, Chunk.BLOCKS_PER_CHUNK, Chunk.BLOCKS_PER_CHUNK, true);
						if(q==0)continue;
						zCounter.setup(this.x<<Chunk.CHUNK_BITS, this.y<<Chunk.CHUNK_BITS, this.z<<Chunk.CHUNK_BITS, z, j, listener, Block.values()[d.get(i)+Block.ID_SHIFT]);
		...
		String worldFolder = WraithavensConquest.saveFolder+File.separatorChar+WraithavensConquest.currentGameUUID;
		CompactBinaryFile file = new CompactBinaryFile(worldFolder, this.x+","+this.y+","+this.z+","+level+".lod");
		file.ensureExistance();
		file.write();
	...
	private boolean isSideShown(int x, int y, int z, int j, byte[] newBlocks, int blockSize, int chunkSize){
		if(j==0){
			...
			if(x==chunkSize){
				...
				for(u = 0; u<blockSize; u++)
					for(v = 0; v<blockSize; v++)
						if(!borderBlocks.getBlock(0, y*blockSize+u, z*blockSize+v, j))return true;
				return false;
			}
		}else if(j==1){
			x--;
			...
			if(y==chunkSize){
				...
				for(u = 0; u<blockSize; u++)
					for(v = 0; v<blockSize; v++)
						if(!borderBlocks.getBlock(x*blockSize+u, 0, z*blockSize+v, j))return true;
				return false;
			}
		}else if(j==3){
			y--;
			...
			if(z==chunkSize){
				...
				for(u = 0; u<blockSize; u++)
					for(v = 0; v<blockSize; v++)
						if(!borderBlocks.getBlock(x*blockSize+u, y*blockSize+v, 0, j))return true;
				return false;
			}
		}else{
			z--;
	 ...
	 * The same as calling:
	 * <br>
	 * <br><code>saveBlocks();</code>
	 * <br><code>saveMesh();</code>
	 */
